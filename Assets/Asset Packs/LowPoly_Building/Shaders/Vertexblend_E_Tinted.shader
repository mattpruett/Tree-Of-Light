...
Shader "LowPoly/Vertexblend_E_Tinted"
{
	Properties
	{
		[HideInInspector] __dirty( "", Int ) = 1
		_TopTexture0("Top Texture 0", 2D) = "white" {}
		_VertexOffsetMap("Vertex Offset Map", 2D) = "white" {}
		_Tint_Gradient("Tint_Gradient", 2D) = "white" {}
		_VertexOffsetXYZ("Vertex Offset XYZ", Vector) = (0,5,0,0)
...
#line 20
		...
		uniform sampler2D _VertexOffsetMap;
		uniform float _PannerSpeed;
		uniform float4 _VertexOffsetXYZ;
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			o.texcoord_0.xy = v.texcoord.xy * float2( 0.0001,1 ) + float2( 0,0 );
			o.texcoord_1.xy = v.texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
			float4 temp_cast_1 = (0.0).xxxx;
			v.vertex.xyz += lerp( ( ( tex2Dlod( _VertexOffsetMap, float4( (abs( o.texcoord_1+( _Time.y * _PannerSpeed ) * float2(0.05,-0.3 ))), 0.0 , 0.0 ) ) * 0.01 ) * _VertexOffsetXYZ ) , temp_cast_1 , v.color.a ).xyz;
		}

		void surf( Input i , inout SurfaceOutputStandard o )
		{
			o.Normal = float3(0,0,1);
			float4 transform100 = mul(unity_ObjectToWorld,float4( 0,0,0,1 ));
			float3 ase_objectScale = float3( length( unity_ObjectToWorld[ 0 ].xyz ), length( unity_ObjectToWorld[ 1 ].xyz ), length( unity_ObjectToWorld[ 2 ].xyz ) );;
			float2 temp_cast_0 = (( ( ( 999.0 * transform100.x ) + ( ( 999.0 * transform100.y ) + ( 999.0 * transform100.z ) ) ) + ( ( ( transform100.z + ase_objectScale.x ) + ( transform100.y + ase_objectScale.z ) ) + ( transform100.x + ase_objectScale.y ) ) )).xx;
			float3 ase_worldPos = i.worldPos;
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float4 triplanar139 = TriplanarSampling( _TopTexture0, _TopTexture0, _TopTexture0, ase_worldPos, ase_worldNormal, 1.0, 0.1, 0 );
			o.Albedo = ( ( saturate( ( ( triplanar139 * i.vertexColor ) > 0.5 ? ( 1.0 - ( 1.0 - 2.0 * ( ( triplanar139 * i.vertexColor ) - 0.5 ) ) * ( 1.0 - tex2D( _Tint_Gradient, ( i.texcoord_0 + temp_cast_0 ) ) ) ) : ( 2.0 * ( triplanar139 * i.vertexColor ) * tex2D( _Tint_Gradient, ( i.texcoord_0 + temp_cast_0 ) ) ) ) )) * 2.0 ).rgb;
			o.Metallic = 0.0;
			o.Smoothness = 0.2;
			o.Alpha = 1;
		}
...
struct v2f_surf {
  ...
  fixed4 color : COLOR0;
  float4 custompack0 : TEXCOORD3; // texcoord_0 texcoord_1
...
v2f_surf vert_surf (appdata_full v) {
  ...
  o.custompack0.zw = customInputData.texcoord_1;
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  ...
  surfIN.texcoord_1.x = 1.0;
  surfIN.texcoord_0 = IN.custompack0.xy;
  surfIN.texcoord_1 = IN.custompack0.zw;
...
struct v2f_surf {
  ...
  fixed4 color : COLOR0;
  float4 custompack0 : TEXCOORD4; // texcoord_0 texcoord_1
  UNITY_LIGHTING_COORDS(5,6)
  UNITY_FOG_COORDS(7)
  UNITY_VERTEX_INPUT_INSTANCE_ID
  UNITY_VERTEX_OUTPUT_STEREO
